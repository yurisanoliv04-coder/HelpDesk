...
export default async function EditarCompraPage({
  ...
  const [purchase, categories, users] = await Promise.all([
    getPurchaseById(id),
    ...
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
          ...
          <span style={{ color: '#1e3048', fontSize: 10 }}>/</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: '#c8d6e5', letterSpacing: '0.1em' }}>EDITAR</span>
        </div>
      ...
      <EditPurchaseForm purchase={purchase} categories={categories} users={users} />
    </div>
  )
}
