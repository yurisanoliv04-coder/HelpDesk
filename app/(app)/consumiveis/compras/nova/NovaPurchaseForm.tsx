...
const labelStyle: React.CSSProperties = {
  fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: '#2d4060', fontWeight: 700, marginBottom: 5, display: 'block',
}
...
export default function NovaPurchaseForm({
  categories,
  users,
}: {
  categories: Category[]
  ...
  const [status, setStatus] = useState<'PENDING' | 'RECEIVED' | 'CANCELED'>('PENDING')
  ...
  const [deferAsset, setDeferAsset] = useState(true)   // EQUIPMENT only
  ...
  const totalPrice = unitPrice && quantity
    ? parseFloat(unitPrice) * parseInt(quantity || '1')
    : null
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const r = await createPurchase({
        ...
        deferAsset: isEquipment ? deferAsset : undefined,
      })
      if (r.ok) {
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5 }}>
              ...
              <input
                ...
                placeholder="NF-e ou nº do pedido" style={iStyle}
              />
            </div>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5 }}>
        ...
        <div style={cardStyle}>
          ...
          {/* ── CARD inline: Categoria & automação ── */}
          <div style={{
            marginTop: 4,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 12 }}>
              <label style={labelStyle}>CATEGORIA DO ITEM {kindChip && <span style={{ marginLeft: 6 }}>{kindChip}</span>}</label>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {isStock && (
                  <div style={{
                    ...
                    ✅ Ao <strong>receber</strong> este pedido, o estoque de <strong>{selectedCat?.name}</strong> será incrementado em <strong>{quantity || 1} unidade(s)</strong> automaticamente.
                  </div>
                )}
                ...
                {isEquipment && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <label style={{
                      ...
                      <input
                        type="radio" name="assetAction" checked={deferAsset}
                        onChange={() => setDeferAsset(true)}
                        style={{ marginTop: 2, accentColor: '#fbbf24' }}
                      />
                      <div>
                        <p style={{ fontSize: 12, fontWeight: 600, color: '#fbbf24', marginBottom: 2 }}>
                          ⏳ Deixar para depois
                        </p>
                        <p style={{ fontSize: 11, color: '#3d5068', lineHeight: 1.4 }}>
                          Cria {quantity || 1} ativo(s) com status <strong style={{ color: '#f87171' }}>IRREGULAR</strong> na aba Patrimônio. O cadastro completo (TeamViewer, specs, etc.) poderá ser feito depois.
                        </p>
                      </div>
                    </label>
                    ...
                    <label style={{
                      ...
                      <input
                        type="radio" name="assetAction" checked={!deferAsset}
                        onChange={() => setDeferAsset(false)}
                        style={{ marginTop: 2, accentColor: '#38bdf8' }}
                      />
                      <div>
                        <p style={{ fontSize: 12, fontWeight: 600, color: '#38bdf8', marginBottom: 2 }}>
                          📋 Cadastrar agora
                        </p>
                        <p style={{ fontSize: 11, color: '#3d5068', lineHeight: 1.4 }}>
                          Salva a compra e redireciona para a tela de cadastro completo do ativo.
                        </p>
                      </div>
                    </label>
                  </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <textarea
              ...
              rows={5} placeholder="Marca, modelo, cor, voltagem, garantia, link do produto..."
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <textarea
              ...
              rows={3} placeholder="Observações sobre o pedido, urgência, etc."
              style={{ ...iStyle, resize: 'vertical' }}
            />
          </div>
        </div>
        ...
        <div style={cardStyle}>
          ...
          {imagePreview ? (
            <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                ...
                <img
                  ...
                  style={{ width: 140, height: 110, objectFit: 'cover', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)' }}
                />
                <button
                  type="button"
              ...
              <div>
                <p style={{ fontSize: 11, color: '#3d5068', lineHeight: 1.5, marginBottom: 8 }}>
                  Foto adicionada com sucesso.<br />Será comprimida automaticamente.
                </p>
            ...
            >
              ...
              <span style={{ fontSize: 11, color: '#1e3048' }}>Nota fiscal, produto ou embalagem · JPG, PNG, WEBP</span>
            </button>
          )}
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
        ...
        >
          {isPending ? 'Salvando...' : isEquipment && !deferAsset ? '→ Salvar & Cadastrar ativo' : '✓ Salvar compra'}
        </button>
        <a
