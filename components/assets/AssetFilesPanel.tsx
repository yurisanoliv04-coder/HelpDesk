...
function FileIcon({ mimeType }: { mimeType: string }) {
  ...
  if (mimeType.includes('zip') || mimeType.includes('rar') || mimeType.includes('7z')) return <FileArchive size={size} color="#fbbf24" />
  if (mimeType.includes('javascript') || mimeType.includes('json') || mimeType.includes('html') || mimeType.includes('css')) return <FileCode size={size} color="#34d399" />
  if (mimeType.includes('word') || mimeType.includes('document')) return <FileText size={size} color="#38bdf8" />
  if (mimeType.includes('sheet') || mimeType.includes('excel') || mimeType.includes('csv')) return <FileText size={size} color="#34d399" />
  if (mimeType.startsWith('text/')) return <FileText size={size} color="#94a3b8" />
  return <File size={size} color="#3d5068" />
}

function mimeLabel(mimeType: string, name: string) {
  ...
  async function handleFiles(fileList: FileList | null) {
    ...
    for (const file of Array.from(fileList)) {
      ...
      startTransition(async () => {
        try {
          await uploadAssetFile(assetId, fd)
          setFiles(prev => [{ id: 'optimistic-' + Date.now(), originalName: file.name, mimeType: file.type || 'application/octet-stream', size: file.size, url: '#', createdAt: new Date(), uploadedBy: { id: currentUserId, name: 'Você' } }, ...prev])
        } catch (e: unknown) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {canEdit && (
        ...
        >
          <div style={{ width: 44, height: 44, borderRadius: 10, background: dragOver ? 'rgba(0,217,184,0.15)' : 'rgba(255,255,255,0.04)', border: `1px solid ${dragOver ? 'rgba(0,217,184,0.3)' : 'rgba(255,255,255,0.07)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
      ...
      {error && (
        <div style={{ background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#f87171', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          {error}
      ...
      {files.length === 0 ? (
        <div style={{ background: '#0d1422', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '40px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          ...
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: '#1e3048', fontStyle: 'italic' }}>Nenhum arquivo anexado</p>
        </div>
      ) : (
              ...
              <div key={file.id} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 130px 120px 72px', alignItems: 'center', padding: '12px 16px', background: i % 2 === 0 ? 'rgba(255,255,255,0.01)' : 'transparent', borderTop: i === 0 ? 'none' : '1px solid rgba(255,255,255,0.03)', opacity: isDeleting ? 0.3 : 1, transition: 'opacity 0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontSize: 12, fontWeight: 600, color: '#c8d6e5', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.originalName}</p>
                    <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: '#2d4060', marginTop: 1 }}>{fmtDate(file.createdAt)}</p>
                  </div>
                </div>
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, fontWeight: 700, color: '#3d5068' }}>{mimeLabel(file.mimeType, file.originalName)}</span>
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: '#3d5068' }}>{fmtSize(file.size)}</span>
